...
export function UrlPreview({ data, loading, error, onRetry, className }: UrlPreviewProps) {
  ...
  return (
    <div className={`border rounded-lg p-4 bg-white dark:bg-gray-800 shadow-sm hover:shadow-md transition-shadow ${className || ''}`}>
        ...
        <div className="mb-3 p-2 bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded text-xs">
          <div className="flex items-center">
            ...
            <span className="text-yellow-800 dark:text-yellow-200">
              {data.fallback ? 'Limited preview - website blocking prevented full content extraction' : error}
            </span>
          </div>
        </div>
      )}
